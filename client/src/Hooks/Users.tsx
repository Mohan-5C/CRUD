...
// import { addUser, deleteUser } from "../Redux/Slices/userReducer";
...
export const Users:React.FC<UserComponentProps> = ({ user, setUser }) => {
  // const [users,setUsers]=useState([]);
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  const users = useSelector((state) => (state as any).users.users);
  const { loading, error } = useSelector((state) => (state as any).users);
